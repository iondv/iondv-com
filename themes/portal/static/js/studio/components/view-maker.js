...
$.extend(Studio.ViewMaker.prototype, {
  ...
  onChangeContentMode: function (event, mode) {
    this.$container.toggle(mode === 'view');
    this.redraw();
  },
  ...
  redraw: function (activeViewTabId) {
    if (this.isActive()) {
      this.view = this.studio.getActiveClassView();
  ...
  draw: function (activeViewTabId) {
    ...
    this.renderSource();
    if (!this.view) {
      content = this.studio.renderAlertSample('warning', 'Select a view to display elements');
