...
$.extend(Studio.SidebarMenu.prototype, {
  ...
  getItemByType: function (type, $parent) {
    return $parent.find('[data-type="'+ type +'"]');
  },
  ...
  setValidContentMode: function ($item) {
    ...
    var mode = this.getValidContentModeByType(type);
    this.studio.setContentMode(mode);
  },

  getValidContentModeByType: function (type) {
    var modes;
    switch (type) {
      case 'class':
      case 'classAttr':
  ...
  getActiveAppSection: function (type) {
    return this.getActiveAppItem().find('.app-section[data-type="'+ type +'"]');
  },
  ...
  getActiveAppItem: function () {
    return this.getClosestByType('app', this.getActive());
  },
  ...
  createAppSections: function (model) {
    return [{
      ...
      'type': 'algorythms',
      'title': Helper.L10n.translate('Algorythms'),
      'children': this.createAlgorythms(model)
    }, {
      'type': 'tasks',
      ...
      'type': 'admin',
      'title': Helper.L10n.translate('Administration'),
  ...
  getActiveClassItem: function () {
    return this.getClosestByType('class', this.getActive());
  },
  ...
  createAlgorythms: function (app) {
    return app.algos.map(function (model) {
      return this.createAlgorythm(model);
    }, this).join('');
  },

  createAlgorythm: function (model) {
    return this.createItem({
      'type': 'algorythm',
      'id': model.id,
      'name': model.getName(),
  ...
  getActiveAlgo: function () {
    return this.getAlgoByItem(this.getActive());
  },

  getAlgoByItem: function ($item) {
    var $algo = this.getClosestByType('algorythm', $item);
    var $app = this.getClosestByType('app', $algo);
    return this.studio.getAlgo($algo.data('id'), $app.data('id'));
  },

  onCreateAlgo: function (event, model) {
    var $item = this.getItemByType('algorythms', this.getItem(model.app.id));
    this.getChildren($item).append(this.createAlgorythm(model));
    this.activate(this.getItem(model.id));
  },
