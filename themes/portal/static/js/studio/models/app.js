...
$.extend(Studio.AppModel.prototype, Studio.Model.prototype, {
  ...
  createDefaultClassViews: function () {
    var views = {};
    this.classes.forEach(function (cls) {
      cls.clearViews();
      cls.createDefaultViews({views});
      cls.createViewAttrsByClass();
    });
  },
  ...
  createAlgos: function (items) {
    if (items instanceof Array) {
      items.forEach(this.createAlgo, this);
    }
  },

  createAlgo: function (data) {
    return this.createNestedModel(data, this.algos, this.algoMap, Studio.AlgoModel);
  },

  removeAlgo: function (model) {
    this.removeNestedModel(model, this.algos, this.algoMap);
    this.studio.triggerRemoveAlgo(model);
  },
  ...
  createNavSectionByClasses: function () {
    this.clearNavSections();
    this.createNavSection({
      'name': 'main',
      'caption': 'Main',
      'orderNumber': 0,
      'mode': 0,
      'tags': ''
    }).createItemsByClasses(this.classes);
  },
  ...
  exportData: function () {
    ...
    data.algos = Helper.Array.mapMethod('exportData', this.algos);
